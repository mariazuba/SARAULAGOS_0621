...
x0      <-c(years1,rev(years1))
x0_1    <-c(2000,2022,22) #xaxp

#######################################################################################
#modelo base ASESORÍA SEPTIEMBRE
#######################################################################################
Ro0           <- c(subset(std0,name=="log_Rmed")$value,0)
Reclutas0     <- c(subset(std0,name=="Reclutas")$value,0)
Reclutas0std  <- c(subset(std0,name=="Reclutas")$std,0)
logdesvRt0    <- c(subset(std0,name=="log_desv_Rt")$value,0)
logdesvRt0std <- c(subset(std0,name=="log_desv_Rt")$std,0)
Bt0           <- c(subset(std0,name=="BT")$value,0)
Bt0std        <- c(subset(std0,name=="BT")$std,0)
SSBt0         <- c(subset(std0,name=="BD")$value,0)
SSBt0std      <- c(subset(std0,name=="BD")$std,0)
Ft0           <- c(subset(std0,name=="log_F")$value,0)
Ft0std        <- c(subset(std0,name=="log_F")$std,0)
...
#arreglos polígono
rt1      <- c((Reclutas1-1.96*Reclutas1std),
            rev(Reclutas1+1.96*Reclutas1std))
logdrt1  <- c((logdesvRt1-1.96*logdesvRt1std),
            rev(logdesvRt1+1.96*logdesvRt1std))
bt1      <- c((Bt1-1.96*Bt1std),
           rev((Bt1+1.96*Bt1std)))
ssbt1    <- c((SSBt1-1.96*SSBt1std),
           rev((SSBt1+1.96*SSBt1std)))
ft1      <- c(exp((Ft1)-1.96*(Ft1std)),
           rev(exp((Ft1)+1.96*(Ft1std)))) 

```


```{r Fig27,warning=F, include=T, message=F, echo=T,fig.height=8,fig.width=10,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
plot(x0,bt0/1000,type="n",cex.axis=1.1,xaxs="i",yaxs="i",las=1,
	...
	lines(years1,Bt0/1000,lwd=1,col=1,lty=1)
	lines(years1,Bt1/1000,lwd=1,col=2,lty=1)
box()
...
plot(x0,ssbt0/1000,type="n",cex.axis=1.1,xaxs="i",yaxs="i",las=1,
	...
	lines(years1,SSBt0/1000,lwd=1,col=1,lty=1)
	lines(years1,SSBt1/1000,lwd=1,col=2,lty=1)
box()
...
plot(x0, ft0, xaxp=x0_1,cex.axis=1.1,xaxs= "i",yaxs= "i",ylim=c(0,2.5),
	...
	lines(years1,exp(Ft0),lwd=1,col=1,lty=1)
	lines(years1,exp(Ft1),lwd=1,col=2,lty=1)
box()
