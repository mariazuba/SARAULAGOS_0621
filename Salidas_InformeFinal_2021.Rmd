...
### 6.4. Comparación de las tendencias poblacionales del modelo base y alternativo
...
```{r Variables_poblacionales,warning=F, include=T, message=F, echo=T,fig.height=8,fig.width=10,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
par(mfcol=c(2,2),mar=c(2,4,1,1)+0.5)
	plot(x0,rt0/1000 , type="n", xaxp=x0_1,cex.axis=1.1,xaxs= "i",yaxs= "i",
	main="Reclutamientos",ylim=c(0,35),xlim=c(2001,2021),
...
plot(x0,bt0/1000,type="n",cex.axis=1.1,xaxs="i",yaxs="i",las=1,
     xlim=c(2001,2021),xaxp=x0_1,ylim=c(0,450),cex.lab=1.1,
...
plot(x0,ssbt0/1000,type="n",cex.axis=1.1,xaxs="i",yaxs="i",las=1,
     xlim=c(2001,2021),xaxp=x0_1,ylim=c(0,150),cex.lab=1.1,
...
```{r selectividad,warning=F, include=T, message=F, echo=T,fig.height=8,fig.width=10,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r curvasFrms,warning=F, include=T, message=F,eval=F, echo=T,fig.height=4,fig.width=8,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r BD0_BDRMS_BDLIM,warning=F, include=T, message=F, echo=T,fig.height=8,fig.width=10,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
#---------------------------------------------------------------------------------
  ...
  plot(x0,ssbt0/1000,type="n",cex.axis=1.1,xaxs="i",yaxs="i",
       xlim=c(2001,2021),xaxp=x0_1,ylim=c(0,150),las=1,cex.lab=1.1,
...
# MODELO ALTERNATIVO
#---------------------------------------------------------------------------------
	plot(x0,ssbt.0/1000,type="n",cex.axis=1.1,xaxs="i",yaxs="i",xlim=c(2001,2021),
  ...
  text(rep(2014.5,3),(c(BDo.0,BRMS.0,BDlim.0)/1000)+5,c("BDo =","BDrms =","BDlim ="))
	...
	main="Modelo alternativo",xlim=c(2000,2021),type="n", 
...
par(mfcol=c(2,2),mar=c(2,4,1,1)+0.5)
	plot(x0, rpr.0,type="n",ylim=c(0,4),cex.axis=0.8,xaxs="i",yaxs="i",
	     xlim=c(2001,2021),xaxp=x0_1,ylab=expression("BD/BD"[RMS]),
	     las=1,xlab="Año",cex.lab=1.1)
...
plot(x0,frpr.0,type="n",ylim=c(0,5),cex.axis=0.8,xaxs="i",yaxs="i",
     xlim=c(2001,2021),xaxp=x0_1, ylab=expression("F/F"[RMS]),
     las=1,xlab="Año",cex.lab=1.1)                
...
```{r asesorias_previas,warning=F, include=T, message=F, echo=T,fig.height=8,fig.width=10,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
R_jun19   <- c(6215,9079,13095,19689,8096,8467,10623,6528,5133,5375,
             13802,2383,12211,3249,2441,4388,2445,6665,NA)

R_sept19  <- c(6174,9049,13026,19810,8084,8452,10630,6544,5134,
             5369,13770,2410,12176,3261,2505,4861,2735,4690,NA)

BD_jun19  <- c(40355,56370,55954,56952,74917,58016,37351,29081,
              28055,26737,29062,44469,37477,40608,30858,17861,17043,17109,NA)

BD_sept19 <- c(39991,56080,55914,57142,75339,58468,37718,29360,
              28317,26985,29433,44484,37546,40817,31226,18630,19126,18793,NA)

F_jun19   <- c(0.5,0.35,0.4,0.5,0.33,0.6,0.72,0.95,0.38,
             0.33,0.28,0.29,0.35,0.35,0.43,0.49,0.29,0.33,NA)

F_sept19  <- c(0.5,0.35,0.4,0.49,0.33,0.59,0.71,0.94,0.38,
             0.33,0.28,0.3,0.37,0.36,0.43,0.48,0.26,0.34,NA)
...
dat1c <- data.frame(years=years.0,
                    Rt=rep.0b$Reclutamiento,
                    SSBt=rep.0b$Biomasa_desovante,
                    Ft=rep.0b$F)%>% 
...
dat0c <- data.frame(years=years.0,
                    Rt=rep0$Reclutamiento,
                    SSBt=rep0$Biomasa_desovante,
                    Ft=rep0$F)%>% 
...
```{r DiagramaFase_modelobaseJUNIO, warning=F, include=T, message=F, echo=T,fig.height=4,fig.width=4,fig.align="center",out.extra = 'angle=0',fig.path="Figuras/",dev=c('pdf')}

source(paste(getwd(),"/funciones/Fn_DiagramaFase.R",sep="")) 
name1<-"Modelo base Asesoría Junio 2020"
years1<-rep.0b$Years
  
SSBt1         <- subset(std.0b,name=="BD")$value
SSBt1std      <- subset(std.0b,name=="BD")$std
Ft1           <- subset(std.0b,name=="log_F")$value
Ft1std        <- subset(std.0b,name=="log_F")$std
BDo     <- rep.0b$BD_virgen_LP 
BRMS1   <- BDo*0.55
FRMS1   <- data0b$Ind[nyears1,13]
...
```{r DiagramaFase_modelobase, warning=F, include=T, message=F, echo=T,fig.height=4,fig.width=4,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r DiagramaFase_modeloalternativoJUNIO, warning=F, include=T, message=F, echo=T,fig.height=4,fig.width=4,fig.align="center", out.extra = 'angle=0',fig.path="Figuras/",dev=c('pdf')}
...
```{r DiagramaFase_modeloalternativo, warning=F, include=T, message=F, echo=T,fig.height=4,fig.width=4,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
points(lastB1,lastF,pch=19,col=4)
text(lastB1,lastF-0.1,years.0[nyears.0-1],cex=0.8) 
```

### Comparación de la proyección de la Captura Biológicamente Aceptable (CBA inicial - Hito 1)


```{r Rproy_sept2019,warning=F, include=T, message=F, echo=T,fig.height=4,fig.width=7,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r CBAalatalla_CBA2020_sept2019,warning=F, include=T, message=F, echo=T,fig.height=5,fig.width=7,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r Densidad_CBA2020_sept2019,warning=F, include=T, message=F, echo=T,fig.height=2.9,fig.width=7,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r Densidad_CBA2021,warning=F, include=T, message=F, echo=T,fig.height=3,fig.width=7,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
...
```{r Densidad_CBA2020,warning=F, include=T, message=F, echo=T,fig.height=5.8,fig.width=4,fig.align="center",fig.path="Figuras/",dev=c('pdf')}
